  ...
  return (
    <div className={`content content--${darkMode ? "dark" : "light"}`}>
      <div className="sorter container">
        <h1
          className={`sorter__title sorter__title--${
            darkMode ? "dark" : "light"
          }`}
        >
        ...
        <p
          className={`sorter__subTitle sorter__subTitle--${
            darkMode ? "dark" : "light"
          }`}
        >
        ...
        <div
          className={`sorter__group sorter__group--${
            darkMode ? "dark" : "light"
          }`}
        >
          {Array(numberGroups)
                ...
                <div key={i}>
                  ...
                  <div className="sorter__group__list">
                    {groupsSorted.map((pseudo, key) => {
                      if (pseudo.group === i + 1) {
                        return (
                          <p className="sorter__group__list__pseudo" key={key}>
                            {pseudo.name}
                          </p>
                        );
                      } else {
        ...
        <div className={`sorter__input${inputError}`}>
          <label
            className={`sorter__input__label sorter__input__label--${
              darkMode ? "dark" : "light"
            }`}
            htmlFor="nickname"
          >
          ...
          />
          <p>
            Veuillez entrer un pseudo correct (sans commencer par un espace)
          </p>
        </div>

        {listPseudo.length !== 0 ? (
          <>
            <button className="sorter__button" onClick={sortGroup}>
              Valider
            </button>
            <div className="sorter__table">
              {listPseudo.map((pseudo, key) => (
                <div
                  className={`sorter__table__row sorter__table__row--${
                    darkMode ? "dark" : "light"
                  }`}
                  key={key}
                >
